...
from get_position import click_position, move_position
    ...
    try:
        while True:
            ...
            if meg["mode"] == "online":
                ...
                try:
                    ops = client.ask(message22)[0].get('function_call',["没有该元素"]).get('arguments',["没有该元素"])
                except Exception as e:
                    print("没有识别到具体指令")
                    engine.say("没有识别到具体指令")  # 开始朗读
                ...
                move_element = ops.get('move', ["没有该元素"])
                if watch_element != ["没有该元素"]:
                    print(watch_element[0].strip("播放"))
                    # click_position(image_np,watch_element[0].strip("播放"))
                    click_position(watch_element[0].strip("播放"))
                    continue
                if click_element != ["没有该元素"]:
                    print(click_element[0].strip("点击"))
                    # click_position(image_np,click_element[0].strip("点击"))
                    click_position(click_element[0].strip("点击"))
                    continue
                if move_element != ["没有该元素"]:
                    print(move_element[0])
                    move_position(move_element[0])
                    continue
